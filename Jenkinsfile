pipeline {
    ...
    steps{
        sshagent(credentials : ['SSH_USER']) {
            sh 'ssh -o StrictHostKeyChecking=no root@dh.pablomartin.site wget https://github.com/PabloMartin19/django_tutorial/blob/master/docker-compose.yaml -O docker-compose.yaml'
